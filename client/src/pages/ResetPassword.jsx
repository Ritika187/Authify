...
const ResetPassword =() =>{
  ...
  const [isOtpSubmiited, setIsOtpSubmitted] = useState(false);
  ...
  const handleVerify = () =>{
   ...
   if(otp.length !== 6){
    toast.error("Password enter all 6 digits of the OTP.");
    return;
   }
   setOtp(otp);
   ...
   return (
    <div className="email-verify-container d-flex align-items-center justify-content-center vh-100 position-relative" style={{background: "linear-gradient(90deg, #6a5af9, #8268f9", border: "none"}}>
      <Link to="/" className="position-absolute top-0 start-0 p-4 d-flex align-items-center gap-2 text-decoration-none">
      <img src={assets.logo} alt="logo" height={32} width={32} />
      <span className="fs-4 fw-semobold text-light">Authify</span>
      </Link>
        ...
        <div className="rounded-4 p-5 text-center bg-white" style={{width:'100%', maxWidth:'400px'}}>
            ...
            <form onSubmit={onSubmitEmail}>
              <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                <span className="input-group-text bg-tranparent border-0 ps-4">
